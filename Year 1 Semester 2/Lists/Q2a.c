...
List merge(List lst1, List lst2)
{
    ...
    while ((curr1 != NULL) && (curr2 != NULL)){
        ...
        if (*(curr1->dataPtr) > *(curr2->dataPtr)){
            insertDataToEndList(&newLst, *(curr1->dataPtr)); // פונקציה שמקצה זיכרון כנדרש בסעיף זה
            curr1 = curr1->next;
        }

        else{
            insertDataToEndList(&newLst, *(curr2->dataPtr)); // פונקציה שמקצה זיכרון כנדרש בסעיף זה
            curr2 = curr2->next;
        }
    }
